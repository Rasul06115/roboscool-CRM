...
import { LayoutDashboard, Users, BookOpen, DollarSign, Phone, BarChart3, Menu, X, Bot, LogOut, User } from 'lucide-react';
...
export default function App() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-screen bg-gray-100"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-teal-600" /></div>;
  }
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-gray-900 text-white flex flex-col transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-extrabold tracking-tight">RoboSchool</h1>
              <p className="text-xs text-gray-500 font-medium">CRM Pro v2.0</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-white/10">
          ...
          <button onClick={logout} className="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm text-gray-400 hover:text-red-400 hover:bg-white/5 transition-colors">
        ...
        <header className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 bg-teal-50 rounded-lg">
              ...
              <span className="text-sm font-semibold text-teal-700">{user.role}</span>
            </div>
          </div>
        </header>
