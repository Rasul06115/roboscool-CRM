    ...
    <div className="space-y-4 animate-fadeIn">
      <div className="flex flex-col sm:flex-row justify-between gap-3">
        ...
        <div className="flex gap-2">
          <span className="flex items-center text-sm text-gray-500 bg-gray-100 px-3 rounded-xl">{students.length} ta o'quvchi</span>
            ...
            <tbody>{students.map((s, index) => {
              ...
              return (
                <tr key={s.id} className="border-t border-gray-50 hover:bg-gray-50/50">
                  {/* TARTIB RAQAM */}
                  <td className="px-3 py-3 text-gray-400 font-semibold text-center">{index + 1}</td>
                  ...
                  <td className="px-3 py-3">
                    <div><p className="text-sm text-gray-800">{s.fatherName || '—'}</p>
                      {s.fatherPhone && <p className="text-xs text-gray-400 font-mono">{s.fatherPhone}</p>}</div>
                  </td>
                  <td className="px-3 py-3">
                    <div><p className="text-sm text-gray-800">{s.motherName || '—'}</p>
                      {s.motherPhone && <p className="text-xs text-gray-400 font-mono">{s.motherPhone}</p>}</div>
                  </td>
...
function StudentFormModal({ student, groups, onSave, onClose }) {
  const [f, setF] = useState({
    ...
    groupId: student?.groupId || '', balance: student?.balance || 0,
    progress: student?.progress || 0, status: student?.status || 'ACTIVE', notes: student?.notes || '',
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-2xl mx-4 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center px-6 py-4 border-b sticky top-0 bg-white z-10">
          <h3 className="text-lg font-bold">{student ? "Tahrirlash" : "Yangi o'quvchi"}</h3>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100">✕</button>
        </div>
        <div className="p-6 space-y-5">
          <div><h4 className="text-sm font-bold text-gray-700 mb-3">👤 Asosiy</h4>
          ...
          <div><h4 className="text-sm font-bold text-gray-700 mb-3">👨‍👩‍👦 Ota-ona</h4>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2"><label className="block text-xs font-semibold text-gray-600 mb-1">📞 Asosiy tel (SMS) *</label><input className={ic} value={f.parentPhone} onChange={e => setF({...f, parentPhone: e.target.value})} placeholder="+998901234567" /></div>
            </div></div>
          <div><h4 className="text-sm font-bold text-gray-700 mb-3">📚 O'qish</h4>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">Guruh</label>
                <select className={ic} value={f.groupId} onChange={e => setF({...f, groupId: e.target.value})}>
                  <option value="">Tanlang</option>{groups.map(g => <option key={g.id} value={g.id}>{g.course?.icon} {g.name} ({g.schedule})</option>)}
                </select></div>
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">Status</label>
                <select className={ic} value={f.status} onChange={e => setF({...f, status: e.target.value})}>
                  <option value="ACTIVE">Faol</option><option value="INACTIVE">Nofaol</option><option value="GRADUATED">Bitirgan</option>
                </select></div>
          ...
          <div><label className="block text-xs font-semibold text-gray-600 mb-1">📝 Izoh</label>
            <textarea className={`${ic} min-h-[60px]`} value={f.notes} onChange={e => setF({...f, notes: e.target.value})} /></div>
...
function SmsModal({ student, onClose }) {
  ...
  const templates = [
    { label: "To'lov eslatma", text: `RoboSchool: Hurmatli ota-ona, ${student.fullName} ning to'lov muddati yaqinlashmoqda. Summa: ${formatMoney(Math.abs(student.balance))} so'm. Tel: +998901234567` },
    { label: 'Qarz eslatma', text: `RoboSchool: ${student.fullName} bo'yicha ${formatMoney(Math.abs(student.balance))} so'm qarz mavjud. Iltimos to'lovni amalga oshiring.` },
    { label: 'Dars eslatma', text: `RoboSchool: Hurmatli ota-ona, ${student.fullName} ning darsi bugun. Iltimos vaqtida keling.` },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-md mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          <div className="p-3 bg-gray-50 rounded-xl"><p className="font-semibold">{student.fullName}</p><p className="text-xs text-gray-400">{student.group?.name || 'Guruhsiz'}</p></div>
          <div><label className="block text-xs font-semibold text-gray-600 mb-2">Kimga:</label>
            <div className="space-y-2">
              {student.fatherPhone && <label className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 cursor-pointer"><input type="radio" name="phone" value={student.fatherPhone} checked={phone === student.fatherPhone} onChange={e => setPhone(e.target.value)} /><span className="text-sm">👨 {student.fatherName} ({student.fatherPhone})</span></label>}
              {student.motherPhone && <label className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 cursor-pointer"><input type="radio" name="phone" value={student.motherPhone} checked={phone === student.motherPhone} onChange={e => setPhone(e.target.value)} /><span className="text-sm">👩 {student.motherName} ({student.motherPhone})</span></label>}
              <label className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-50 cursor-pointer"><input type="radio" name="phone" value={student.parentPhone} checked={phone === student.parentPhone} onChange={e => setPhone(e.target.value)} /><span className="text-sm">📞 Asosiy ({student.parentPhone})</span></label>
            </div></div>
          <div><label className="block text-xs font-semibold text-gray-600 mb-2">Shablonlar:</label>
            <div className="flex flex-wrap gap-2">{templates.map((t, i) => (
              <button key={i} onClick={() => { setMessage(t.text); setTemplate(t.label); }}
                className={`text-xs px-3 py-1.5 rounded-lg font-semibold ${template === t.label ? 'bg-teal-100 text-teal-700 border border-teal-300' : 'bg-gray-100 text-gray-600'}`}>{t.label}</button>
            ))}</div></div>
          <div><label className="block text-xs font-semibold text-gray-600 mb-1">Xabar</label>
            ...
            <p className="text-xs text-gray-400 mt-1">{message.length}/160</p></div>
          <button onClick={handleSend} disabled={sending} className="w-full py-3 bg-teal-600 text-white rounded-xl text-sm font-semibold hover:bg-teal-700 disabled:opacity-50 flex items-center justify-center gap-2">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          {s.notes && <div><p className="text-xs text-gray-400">📝 Izoh</p><p className="text-sm">{s.notes}</p></div>}
        </div>
      </div>
    </div>
  );
