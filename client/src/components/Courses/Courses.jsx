...
export default function Courses() {
  ...
  const loadData = async () => {
    try {
      ...
      // O'qituvchilar ro'yxatini guruhlardan olish
      const teacherMap = {};
  ...
  const handleDeleteCourse = async (id) => {
    ...
    try { await coursesAPI.delete(id); toast.success("Kurs o'chirildi!"); loadData(); } catch (e) {}
  };
  ...
  const handleDeleteGroup = async (id) => {
    ...
    try { await groupsAPI.delete(id); toast.success("Guruh o'chirildi!"); loadData(); } catch (e) {}
  };
    ...
    <div className="space-y-6 animate-fadeIn">
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div><h3 className="font-bold text-lg text-gray-900">{c.name}</h3><p className="text-sm text-gray-500">{c.description}</p></div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[{ l: 'Yosh', v: c.ageRange || '—' }, { l: 'Davomiylik', v: c.duration || '—' }, { l: 'Narx', v: `${formatMoney(c.price)} so'm` }, { l: "O'quvchilar", v: `${c.studentCount || 0} ta` }].map((item, i) => (
                  <div key={i} className="bg-gray-50 rounded-lg p-3"><p className="text-xs text-gray-400">{item.l}</p><p className="font-bold text-sm text-gray-900">{item.v}</p></div>
                ))}
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-gray-900">Guruhlar ro'yxati</h3>
                ...
                return (
                  <tr key={g.id} className="border-t border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-3 py-3">
                      <span className="text-xs px-2 py-1 rounded-lg bg-blue-50 text-blue-700 font-semibold">{g.weekDays || '—'}</span>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2">
                        <div className="flex-1 bg-gray-200 rounded-full h-2 max-w-[80px]">
                          <div className="h-2 rounded-full transition-all" style={{ width: `${fill}%`, background: fill > 80 ? '#EF4444' : g.course?.color }} />
                        </div>
                    ...
                    <td className="px-3 py-3">
                      <div className="flex gap-1">
                        <button onClick={() => { setEditGroup(g); setShowGroupModal(true); }}
      ...
      {/* Guruh Modal */}
      {showGroupModal && <GroupModal group={editGroup} courses={courses} groups={groups} onSave={handleSaveGroup} onClose={() => { setShowGroupModal(false); setEditGroup(null); }} />}
    </div>
  );
}
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg mx-4 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs font-semibold text-gray-600 mb-1">Yosh oralig'i</label><input className={ic} value={form.ageRange} onChange={e => setForm({...form, ageRange: e.target.value})} placeholder="8-15" /></div>
            ...
            <div><label className="block text-xs font-semibold text-gray-600 mb-1">Narx (so'm)</label><input className={ic} type="number" value={form.price} onChange={e => setForm({...form, price: e.target.value})} placeholder="400000" /></div>
            <div><label className="block text-xs font-semibold text-gray-600 mb-1">Tavsif</label><input className={ic} value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Qisqacha" /></div>
          </div>
          {/* Preview */}
          ...
          <div className="flex justify-end gap-3">
            <button onClick={onClose} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl text-sm font-semibold">Bekor qilish</button>
            <button onClick={() => { if (!form.name) { toast.error('Kurs nomini kiriting!'); return; } onSave({ ...form, price: Number(form.price) || 0 }); }}
...
// ==================== GURUH FORM MODAL ====================
function GroupModal({ group, courses, groups, onSave, onClose }) {
  // O'qituvchilarni guruhlardan olish
  const teacherMap = {};
  groups.forEach(g => { if (g.teacher) teacherMap[g.teacher.id] = g.teacher; });
  ...
  // Jadval matnini avtomatik yaratish
  ...
  const weekDayOptions = [
    ...
    { label: 'Du, Cho, Sha', value: 'Du,Cho,Sha' },
    { label: 'Se, Pay, Ju', value: 'Se,Pay,Ju' },
    { label: 'Har kuni (Du-Sha)', value: 'Du,Se,Cho,Pay,Ju,Sha' },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-w-lg mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-4">
          ...
          <div><label className="block text-xs font-semibold text-gray-600 mb-1">O'qituvchi *</label>
          ...
          <div>
            ...
            <div className="space-y-2">
              {weekDayOptions.map(opt => (
                <label key={opt.value} className={`flex items-center gap-2 p-2.5 rounded-xl cursor-pointer border-2 transition-all ${form.weekDays === opt.value ? 'border-teal-500 bg-teal-50' : 'border-gray-100 hover:bg-gray-50'}`}>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => handleTimeSlot('MORNING')}
                className={`p-3 rounded-xl border-2 text-center transition-all ${form.timeSlot === 'MORNING' ? 'border-teal-500 bg-teal-50' : 'border-gray-200 hover:bg-gray-50'}`}>
                <p className="text-lg">🌅</p>
                <p className="text-sm font-semibold">Ertalab</p>
                <p className="text-xs text-gray-500">08:00 — 10:00</p>
              </button>
              <button onClick={() => handleTimeSlot('AFTERNOON')}
                className={`p-3 rounded-xl border-2 text-center transition-all ${form.timeSlot === 'AFTERNOON' ? 'border-teal-500 bg-teal-50' : 'border-gray-200 hover:bg-gray-50'}`}>
                <p className="text-lg">🌤</p>
                <p className="text-sm font-semibold">Kunduzi</p>
          ...
          <div className="flex justify-end gap-3">
            <button onClick={onClose} className="px-5 py-2.5 bg-gray-100 text-gray-700 rounded-xl text-sm font-semibold">Bekor qilish</button>
            <button onClick={() => {
              if (!form.name || !form.courseId || !form.teacherId) { toast.error("Barcha majburiy maydonlarni to'ldiring!"); return; }
